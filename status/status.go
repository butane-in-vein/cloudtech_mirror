package status

import (
	"assignment1/structs"
	"encoding/json"
	"log"
	"net/http"
	"time"
)

func GetStatus(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		// log.Println("Get") // idk what log message to put
		w.Header().Set("Content-Type", "application/json")
		countryResp, err := http.Get("http://129.241.150.113:8080/v3.1/name/norway")
		if err != nil {
			log.Fatal("not valid country endpoint, try again")
			http.Error(w, "Not a real country", http.StatusBadRequest)
		}
		countryStatus := countryResp.Status

		currencyResp, err := http.Get("http://129.241.150.113:9090/currency/NOK")
		if err != nil {
			log.Fatal("not valid currency endpoint")
		}

		currencyStatus := currencyResp.Status

		// PLACE UPTIME HERE ?
		// TODO: UPTIME

		currentStatus := structs.StatusStruct{
			CountryRestApi:  countryStatus,
			CurrencyRestApi: currencyStatus,
			Version:         "v1",
			Uptime:          time.Now().String(), // last one kinda sucks, change it properly
		}
		//encode := json.NewEncoder(w)
		b, err := json.Marshal(currentStatus)

		if err != nil {
			http.Error(w, "JSON encoding problem:"+err.Error(), http.StatusInternalServerError)
		} else {
			_, err = w.Write(b)
			if err != nil {
				return
			}
		}

	default:
		http.Error(w, "Nice try, this does not exists", http.StatusNotImplemented)
		return
	}

}
